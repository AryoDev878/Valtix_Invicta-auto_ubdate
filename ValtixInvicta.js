...
/* ========== AUTO-UPDATE ON FIRST-RUN (VPS / PTERODACTYL READY) ========== */
(async () => {
  try {
    console.log("[FIRST-RUN] Checking latest version...");
    const localHash = crypto.createHash("md5").update(fs.readFileSync(__filename)).digest("hex");
    const remoteScript = await new Promise((res, rej) =>
      https.get(UPDATE_CONFIG.GITHUB_RAW_URL, (r) => {
        let d = "";
        r.on("data", (c) => (d += c));
        r.on("end", () => res(d));
      }).on("error", rej)
    ...
    if (localHash !== remoteHash) {
      console.log("[FIRST-RUN] Update available. Downloading & replacing...");
      ...
      fs.writeFileSync(__filename, obfuscated, "utf8");
      console.log("[FIRST-RUN] Update applied. Restarting...");
      setTimeout(() => process.exit(0), 1000);
      return; // jangan lanjutkan script lama
    }
    console.log("[FIRST-RUN] Already latest version. Continue...");
  } catch (err) {
    console.log("[FIRST-RUN] Gagal cek/update:", err.message);
  }
})();
/* ========== END FIRST-RUN UPDATE ========== */
