...
Klik tombol di bawah untuk langsung update.`, {
      chat_id: chatId,
      message_id: messageId,
      parse_mode: "Markdown",
      reply_markup: {
        inline_keyboard: [
          [{ text: "⊱ 𝗨𝗽𝗱𝗮𝘁𝗲 𝗦𝗲𝗸𝗮𝗿𝗮𝗻𝗴 ⊰", callback_data: "update_now" }],
          [{ text: "⊱ 𝗧𝘂𝘁𝘂𝗽 ⊰", callback_data: "close_msg" }]
        ]
      }
    }
  );
...
Mohon tunggu ±30 detik.`, {
  ...
  await performUpdate(chatId);   // fungsi update kamu
}
...
setInterval(async () => {
    try {
        ...
        if (remoteVersion.version !== localVersion.version || localHash !== remoteHash) {
            const premiumChats = premiumUsers
                .filter(u => new Date(u.expiresAt) > new Date())
                .map(u => u.id);

            for (const uid of premiumChats) {
                try {
                    await bot.sendMessage(uid,
                        ...
                        `⚡ Pilih aksi di bawah:`,
                        {
                            ...
                            reply_markup: {
                                inline_keyboard: [
                                    [
                                        { text: "⊱ 𝗟𝗶𝗵𝗮𝘁 𝗗𝗲𝘁𝗮𝗶𝗹 ⊰", callback_data: "detail_update" },
                                        { text: "⊱ 𝗨𝗽𝗱𝗮𝘁𝗲 𝗦𝗲𝗸𝗮𝗿𝗮𝗻𝗴 ⊰", callback_data: "update_now" }
                                    ],
                                    [{ text: "⊱ 𝗧𝘂𝘁𝘂𝗽 ⊰", callback_data: "close_msg" }]
                                ]
                    ...
                    console.log(`Gagal notif ke ${uid}:`, e.message);
                }
            }
        }
    } catch (err) {
...
}, 10 * 1000);   // 30 detik, ganti sesukamu
...
setTimeout(async () => {
    try {
        ...
        if (remoteVersion.version !== localVersion.version) {
            console.log(chalk.yellow(`⚠️  Update tersedia! ${localVersion.version} → ${remoteVersion.version}`));
        }
    } catch (e) {
